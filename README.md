# Dev Forum

Dev Forum is an online platform where software developers can showcase their portfolios, bid on projects, and connect with recruiters to get hired.

<img alt='screenshot' src='https://mir-s3-cdn-cf.behance.net/project_modules/1400/4186cb121098563.60bf274acd7eb.png' />

Live: https://dev-forum-rho.vercel.app

_Note: This project is currently under development, with a migration to the Next.js App Router underway. You can check out the master branch to view the older version of the project._
