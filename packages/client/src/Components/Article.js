        ...
        {!details ? (
          ...
          <>
            ...
            <div className={`flex mt-2 items-center text-xs`}>
              <span className="pl-1 bg-gray-200 mr-2 font-semibold text-xs text-gray-500 py-.5 px-1 rounded mb-1">
                Tags :
              </span>
              {article?.tags?.map((tag, idx) => (
                <span
                  key={idx}
                  className="pl-1 dark:bg-gray-700 bg-gray-200 mr-2 text-xs text-gray-500 dark:text-gray-300 py-.5 px-1 rounded mb-1"
                >
                  #{tag}
                </span>
              ))}
